...
High Availability can be accomplished by increasing the number of replicas for the deployment of Prometheus. Thanos can be installed separately as a [maintained package](../../migration/thanos-maintained.md). Its Prometheus sidecar and object storage must be configured explicitly in Monitoring values.
```yaml
monitoring:
  values:
    thanosRuler:
