...
function process_arguments
{
  while [ -n "$1" ]; do # while loop starts
    ...
    -i) echo "-i option passed to send init script to k3d instance"
        shift
        INIT_SCRIPT=$1
    ;;

    -H) echo "-H option passed to use existing system public IP"
        shift
        PublicIP=$1
    ;;

    -P) echo "-P option passed to use existing system private IP"
        shift
        PrivateIP=$1
    ;;

    -U) echo "-U option passed to provide username for connecting to existing system"
        shift
        SSHUSER=$1
    ;;

    -k) echo "-k option passed to provide SSH key for connecting to existing system"
        shift
        SSHKEY=$1
    ;;

    -c) echo "-c option passed to specify cloud provider"
        shift
        CLOUDPROVIDER=$1
    ;;
