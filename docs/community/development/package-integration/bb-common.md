...
- Set `istio.authorizationPolicies.generateFromNetpol: true` to generate authorization policies from network-policy rules
...
- Add package-specific policies through `istio.authorizationPolicies.custom`; use the bb-common documentation as the source of truth for supported fields
