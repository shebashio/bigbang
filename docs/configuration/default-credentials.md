...
Garage `0.9.3-bb.0` does not support an existing Secret or automatic generation
for its S3 access key and admin token, so the test profile supplies fixed,
ephemeral values. The resulting values are stored in the
`garage/garage-credentials` Secret:

| Garage credential | Secret key | Retrieval command |
| ----------------- | ---------- | ----------------- |
| S3 access-key ID | `accessKeyId` | `kubectl -n garage get secret garage-credentials -o go-template='{{base64decode .data.accessKeyId}}'` |
| S3 secret key | `secretAccessKey` | `kubectl -n garage get secret garage-credentials -o go-template='{{base64decode .data.secretAccessKey}}'` |
| Admin API token | `adminToken` | `kubectl -n garage get secret garage-credentials -o go-template='{{base64decode .data.adminToken}}'` |
