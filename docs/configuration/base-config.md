...
| istio.egressGateway.enabled | bool | `false` | Enable the shared egress waypoint gateway (requires ambient mode). This automatically enables the istioEgressGateway package. |
...
| routes.defaults.inbound.gateways | list | `["istio-gateway/public-ingressgateway"]` | Default Istio gateways for package inbound routes that do not define their own |
| routes.defaults.outbound.egressGateway | string | `"istio-egress/egress-waypoint"` | Default egress waypoint binding ('<namespace>/<name>') for package outbound routes. Automatically blanked when ambient mode or the istioEgressGateway package is disabled. |
