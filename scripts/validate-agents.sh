...
if [[ -n "$repo_root" ]] && git -C "$repo_root" check-ignore --no-index -q "$agents_path"; then
...
while IFS= read -r markdown_link; do
  target=${markdown_link:2:${#markdown_link}-3}
